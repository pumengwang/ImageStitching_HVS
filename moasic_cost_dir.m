...
moasic_points = begin;
while(i1 <= terminal(1,1)&& j1 <= terminal(1,2))
...
%                   cost_map(i1+1,j1-1),  cost_map(i1+1,j1),  cost_map(i1+1,j1+1)];  
        ...
        window = [ cost_map(i1,j1),    cost_map(i1,j1+1);
                   cost_map(i1+1,j1),  cost_map(i1+1,j1+1)];
        pos_window = [  i1,j1; i1,j1+1;  i1+1,j1; i1+1,j1+1];
...
%             j1 = terminal(1,2);
        ...
        ref_window = uint8(a-reshape(ref_candients,2,2)');%%%与window矩阵对应，该矩阵为0代表不考虑该坐标
        window = double(window.*ref_window);%%将窗口内不考虑的点权值置1
        
        window_length = [    norm([i1,j1]-terminal),     norm([i1,j1+1]-terminal);
                          norm([i1+1,j1]-terminal),   norm([i1+1,j1+1]-terminal)];
        ...
        un_candients = [un_candients;[i1,j1];[i1-h,j1-l];];%%根据选择点的方向得到十字非候选点
